...
## 2.为什么要用索引
```
索引就是为了提高查询效率,就像书的目录.其实说白了,索引要解决的就是查询慢的问题
```

## 3.索引的优势和劣势
```
优势：
    索引大大减小了服务器需要扫描的数据量,索引可以帮助服务器避免排序和临时表,索引可以将随机IO变成顺序IO,
    从而提高查询速度.
劣势：
...
2. 唯一索引: 索引列的值必须唯一,但允许NULL值,如果是组合索引,则列值的组合必须唯一
...
如何解决hash冲突：多次散列、链式存储
```

### 5.2.btree索引
```
B-Tree通常意味着所有的值都是按顺序存储的,并且每一个叶子页到根的距离相同,很适合查找范围数据
B-Tree 索引是 MySQL 数据库中使用最为频繁的索引类型.不仅仅在 MySQL 中是如此,实际上在其他的很
